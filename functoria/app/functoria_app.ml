    ...
    with_output file
      (fun oc () ->
         ...
         Codegen.append fmt "(* %s *)"
           (Codegen.generated_header ~time:(Info.build_time i) ());
         Codegen.newline fmt;
...
module Make (P: S) = struct
  ...
  let init_global_state argv =
    ...
    in
    let time =
      match Cmdliner.Term.eval_peek_opts ~argv Cli.time  with
      | (None | Some None), _ -> None
      | Some (Some t), _ -> Some t
    in
    { time; config_file; build_dir }
  ...
  let get_build_time state = match state.time with
...
(include dune.config)\n\n(include dune.build)\n"
  ...
  let generate_dune_project ~project_root =
    ...
    let contents = Fmt.strf "(lang dune 1.6)\n%s\n" auto_generated in
    generate ~file ~contents
  ...
  let build_and_execute ~state ?help_ppf ?err_ppf argv =
    ...
    generate_configuration_files ~state
  ...
  let configure_main ~state ~argv i jobs =
    ...
    Codegen.append_main "(* %s *)"
      (Codegen.generated_header ?time:state.time ());
    Codegen.newline_main ();
...
module DSL (P: S) = struct
  ...
  let register ?packages ?keys ?(init=default_init) name jobs =
    ...
    let build_dir = get_build_dir state in
    let build_time = get_build_time state in
    let main_dev = P.create (init @ jobs) in
    let c = Config.make ?keys ?packages ~init name build_dir build_time main_dev in
