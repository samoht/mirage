...
let fold (type acc) ~if_ ~app ~dev (acc : acc) t =
  let tbl = Tbl.create 50 in
  let rec aux : type a. acc -> a t -> acc =
   fun acc impl ->
    if Tbl.mem tbl @@ abstract impl then Tbl.find tbl (abstract impl)
      ...
      let acc =
        ...
        | Dev d ->
            let deps =
              List.fold_right
                (fun (Abstract x) l -> aux acc x :: l)
                (Device.extra_deps d) []
            in
            dev.f d ~deps
        | App a ->
            let f = aux acc a.f in
            let x = aux f a.x in
            app ~f ~x
        | If (cond, then_, else_) ->
            let then_ = aux acc then_ in
            let else_ = aux then_ else_ in
