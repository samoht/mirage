...
val configure : ('a, 'b) t -> Info.t -> effect
...
val extend :
  ?packages:Package.t list ->
  ?packages_v:Package.t list Key.value ->
  ?pre_configure:(Info.t -> effect) ->
  ?post_configure:(Info.t -> effect) ->
  ?pre_build:(Info.t -> effect) ->
  ?post_build:(Info.t -> effect) ->
  ('a, 'b) t ->
  ('a, 'b) t
